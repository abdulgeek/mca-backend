...
const enrollValidation = [
  ...
  // Conditional validation for biometric data
  body().custom((body) => {
    const { faceImage, fingerprintData, externalFingerprintData } = body;

    // At least one biometric method must be provided
    ...
    // If faceImage is provided, validate it
    if (faceImage) {
      if (typeof faceImage !== "string" || faceImage.length === 0) {
        throw new Error("Face image must be a non-empty string");
      }
      if (!faceImage.match(/^data:image\/(jpeg|jpg|png);base64,/)) {
        throw new Error("Face image must be a valid base64 encoded image");
      }
    }
...
const studentAttendanceValidation = [
  param("studentId")
    ...
    .matches(/^[A-Z0-9\-]+$/)
    .withMessage(
      "Student ID must contain only uppercase letters, numbers, and hyphens"
    ),
  query("startDate")
    .optional()
    .isISO8601()
